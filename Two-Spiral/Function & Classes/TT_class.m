classdef TT_class
    ...
    properties
        ...
        LastPerfMethod;      % The last performed method
    end
    ...
    methods
        ...
        function TT = TTV_method(Data,n,d,TT)
            ...
            for k=1:TT.NumCores-1
                ...
                if r(k+1) == 0  % want the minimum interconnection rank to be equal to 1, (in order to work) 
                   r(k+1) = 1;  
                end
                
                Error_SVs(k) = sum(S(r(k+1)+1:1:end).^2,'all');     % all the truncated SVs^2
            ...
            TT.SV_squared = sum(S(1:1:r(k+1)).^2,'all');  % sum of the last core sv^2s (norm)
            TT.LastPerfMethod = 'TTV_method';
        ...
        function TT = ContractTwoTT(TT1,TT2,IndexContraction_A,IndexContraction_B)
            ...
            if ErrorCheck ==0
                ...
                for i=1:TT1.NumCores
                    ...
                    TT.LastPerfMethod = 'ContractTwoTT';
                    TT.Error_SV_squared = 'unknown - perform rounding';
                    TT.SV_squared = 'unknown - perform rounding';
                end   
            else 
        ...
        function TT = OuterProductTwoTTV(TT1,TT2) 
            ...
            if TT1.TTtype == 1 && TT2.TTtype == 1 && TT1.NumCores == TT2.NumCores
                ...
                TT.Error_SV_squared = 'unknown - perform rounding';  % outer product -> 
                TT.SV_squared = 'unknown - perform rounding';         % Norm (the centered singular values in last core)
                TT.LastPerfMethod = 'OuterProductTwoTTV';    
            else 
        ...
        function TT = GenRankOneTT(n,d,TTtype,scaling) 
            ...
            if TT.TTtype    == 1
                ...
                for i=1:TT.NumCores
                    ...
                    TT.SV_squared = (scaling^2)*(n^d);
                    TT.Error_SV_squared = 0;
                end
            elseif TT.TTtype == 2
                TT.NumCores = 0.5*d;
                for i=1:TT.NumCores
                    ...
                    TT.SV_squared = (scaling^2)*(n^(0.5*d));
                    TT.Error_SV_squared = 0;
                end
            end
            ...
            TT.LastPerfMethod = 'GenRankOneTT';
        end 
    end
